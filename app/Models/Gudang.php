...
class Gudang extends Model
{
    ...
    // Fungsi untuk update data kamar menggunakan Eloquent
    public function updateKamar($data, $id)
    {
        return $this->where('id_kamar', $id)->update($data);
    }

    // Fungsi untuk menampilkan semua data dari tabel yang diberikan (menggunakan Query Builder Laravel)
    public function tampil($tabel)
    {
        return DB::table($tabel)->get();
    }

    // Fungsi untuk menghitung jumlah data pada tabel tertentu
...
public function getWhere($table, $where)
{
    ...
    public function getAlumni($table, $where = [])
{
    return $this->db->table($table)->where($where)->get()->getResult();
}

public function join($tabel, $tabel2, $kolom1, $kolom2)
{
    return DB::table($tabel)
             ->join($tabel2, $kolom1, '=', $kolom2)
             ->get(); // Mengembalikan hasil sebagai koleksi
}
public function joinmenu($tabel, $tabel2, $on)
{
    [$first, $second] = explode(' = ', $on); // Memisahkan kolom kiri dan kanan dari string `$on`
    return DB::table($tabel)
             ->join($tabel2, $first, '=', $second) // Memetakan kolom dengan format yang benar
             ->get();
}
...
public function joinThreeTables($tabel, $tabel2, $tabel3, $on1Column1, $on1Column2, $on2Column1, $on2Column2) {
    return DB::table($tabel)
        ->join($tabel2, function($join) use ($on1Column1, $on1Column2) {
            $join->on($on1Column1, '=', $on1Column2);
        })
        ->join($tabel3, function($join) use ($on2Column1, $on2Column2) {
            $join->on($on2Column1, '=', $on2Column2);
        })
        ->get();
}
...
{
    DB::table($table)->where($kolom, $id)->update($data);
}
    ...
    public function getTotalPengeluaran()
{
    ...
    public function clear_table($table)
    {
        ...
        DB::table($table)->truncate(); // Gunakan truncate untuk menghapus semua data
    }
}
